...
class ClassRoomServiceImpl()(implicit ec: ExecutionContext)
    extends ClassRoomService {
  ...
  override def createClassRoom: ServiceCall[CreateClassRoom, ClassRoom] =
    ServiceCall { request: CreateClassRoom =>
      val classRoomUuid: UUID = UUID.randomUUID()

      classRooms
        .find(
          _.className.toLowerCase.equals(request.className.toLowerCase)
        ) match {
        case Some(_) =>
          throw new Exception(s"classroom ${request.className} already exists")
        case None =>
  ...
  override def deleteClassRoom(
      classRoomUuid: UUID
  ): ServiceCall[NotUsed, ClassRoom] =
    ServiceCall { _ =>
  ...
  override def listClassRooms: ServiceCall[NotUsed, List[ClassRoomDetails]] =
    ServiceCall { _ =>
      Future {
  ...
  override def startClassRoomSession(
      classRoomUuid: UUID
  ): ServiceCall[NotUsed, ClassRoom] =
    ServiceCall { _ =>
  ...
  override def endClassRoomSession(
      classRoomUuid: UUID
  ): ServiceCall[NotUsed, ClassRoom] =
    ServiceCall { _ =>
